    ...
    <div className="flex py-6 flex-col justify-end items-center gap-12 md:gap-3 self-stretch">
      {texts.map((text, index) =>
        index === 0 ? (
          <div
            key={index}
            className="flex flex-col md:flex-row items-start gap-8 self-stretch">
            <h1 className="font-Helloviteca flex-1 text-primary-purple text-[88px]">
              {text.section}
            </h1>
          ...
          <div
            key={index}
            className="flex flex-col md:flex-row  items-start gap-2 md:gap-8 self-stretch">
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        answer: `If you are currently an undergraduate, master’s student (from any university) or NUFYP student, then yes — you are eligible! We consider the 2025–2026 academic year, so new graduates are <strong>not</strong> eligible.
Please also note that participants must be under 25 years old at the time of application.`,
      },
      {
        question:
