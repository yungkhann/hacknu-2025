...
const Agenda = () => {
    return (
    <div className = "py-24 px-2 flex flex-col md:flex-row gap-x-6">
            ...
            {AgendaData.map((item: IAgendaData, index: number) => {
                return (
                    <div className="flex flex-col">
                        <div className="flex items-center"><span className="text-secondary-grey pr-3">#</span><Icons.separator/></div>
                            <div
                                key={index}
                                className="py-4 flex flex-col lg:flex-row">
                                <div className="py-4 w-full lg:w-2/5">
                                    {item.weekday}, {item.date} 
                                </div>
                                <div className="py-4 w-full lg:w-3/5 leading-[2]">
                                    {item.events.map((event, event_index) => (
                                    <div key={event_index} className="flex flex-row justify-between">
                                        <div className={`text-primary-${event_index === 0 ? 'green' : 'grey'}`} ><span className="opacity-60">{">"}</span> {event.name}</div>
